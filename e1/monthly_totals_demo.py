...
def pivot_months_pandas(data):
    ...
    # monthly_grouped_data = data.groupby(['name', 'month']).aggregate({'precipitation': 'sum'}).reset_index()
    monthly_grouped_data = data.groupby(['name', 'month']).aggregate({'precipitation': 'sum'}).reset_index()

    final_monthly = monthly_grouped_data.pivot(index='name', columns='month', values='precipitation')

    counts_grouped_data = data.groupby(['name','month']).aggregate({'precipitation': 'count'}).reset_index()
    final_counts = counts_grouped_data.pivot(index='name', columns='month', values='precipitation')

    return counts_grouped_data, final_counts
    # return final_monthly, final_counts
