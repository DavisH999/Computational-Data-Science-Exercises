...
def pivot_months_pandas(data):
    ...
    """
    # Applying the date_to_month function to date column to make month column
    print(data)
    data['month'] = data['date'].apply(date_to_month)
    grouped_data = data.groupby(['name', 'month'])
    monthly = grouped_data.aggregate({'precipitation': 'sum'}, numeric_only=True).\
        reset_index().\
        pivot(index='name', columns='month', values='precipitation')

    # Making the count for each group
    counts = grouped_data.aggregate('count').\
        reset_index().\
        pivot(index='name', columns='month', values='precipitation')
